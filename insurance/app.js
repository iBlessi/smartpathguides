...
function showResults() {
  ...
  recs.push({ name: "Health Insurance", reason: employment === "freelance" || employment === "business-owner" ? "As self-employed, you must secure your own health coverage." : "Health insurance is essential. Review your employer's plan options.", link: "#health", priority: "essential" });

  if (housing === "owner") {
    recs.push({ name: "Homeowners Insurance", reason: "Protects your largest asset. Your mortgage lender likely requires it.", link: "#home-ins", priority: "essential" });
  } else if (housing === "renter") {
    recs.push({ name: "Renters Insurance", reason: "Your landlord's policy doesn't cover your belongings. At $15-30/month, it's one of the best values.", link: "#renters", priority: "essential" });
  }

  if (dependents !== "none") {
    recs.push({ name: "Life Insurance", reason: `With ${dependents === "one" ? "a partner" : "a family"} depending on your income, term life insurance is critical.`, link: "#life", priority: "essential" });
  }

  const list = document.getElementById("rec-list");
  const priorityLabels = { essential: "Essential", recommended: "Recommended", consider: "Worth Considering" };
  list.innerHTML = recs.map(r => `<li><strong><a href="${r.link}" style="color:var(--forest);text-decoration:underline;">${r.name}</a> — ${priorityLabels[r.priority]}</strong><span>${r.reason}</span></li>`).join("");

  document.getElementById("quiz-container").style.display = "none";
  document.getElementById("quiz-results").style.display = "block";
  document.querySelectorAll("#quiz-progress .step").forEach(s => { s.className = "step done"; });
}
